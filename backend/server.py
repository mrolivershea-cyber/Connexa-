...
async def manual_fraud_test_batch(
    ...
    # Запускаем в background с правильной обработкой
    task = asyncio.create_task(process_fraud_test_background(session_id, node_ids, None))
    logger.info(f"🚀 Created fraud test background task for session {session_id}")
...
async def manual_geo_fraud_test_batch(
    ...
    # Запускаем в background с правильной обработкой
    task = asyncio.create_task(process_geo_fraud_test_background(session_id, node_ids, None))
    logger.info(f"🚀 Created geo+fraud test background task for session {session_id}")
